...
type FramingMode = "jsonl" | "content-length";

type BridgeConnection = {
  url: string;
  token: string;
  product?: string;
  unityExplorerVersion?: string;
};

const toolToBridgeMethod: Record<string, string> = {
  ue_status: "status",
...
function loadConnection(): BridgeConnection {
  ...
  if (!existsSync(connectionFile)) {
    throw new Error(
      `UnityExplorer MCP connection file was not found at ${connectionFile}. ` +
      `Start a game with the bridge enabled, or pass --url and --token.`
    );
  }

  const parsed = JSON.parse(readFileSync(connectionFile, "utf8")) as BridgeConnection;
...
async function callBridge(method: string, params: any): Promise<any> {
...
async function handleRequest(request: JsonRpcRequest, framing: FramingMode): Promise<void> {
  try {
...
function send(message: unknown, framing: FramingMode = "jsonl") {
...
function sendError(id: string | number | null | undefined, code: number, message: string, framing: FramingMode = "jsonl") {
  send({
    jsonrpc: "2.0",
    id: id ?? null,
    error: { code, message }
...
function detectFraming(bytes: Buffer): FramingMode {
...
function readContentLengthFrame(): string | null {
...
function readJsonLineFrame(): string | null {
...
async function dispatchRawMessage(body: string, framing: FramingMode) {
  try {
    await handleRequest(JSON.parse(body) as JsonRpcRequest, framing);
  } catch (error) {
    sendError(
      null,
      -32700,
      error instanceof Error ? `Invalid JSON-RPC message: ${error.message}` : "Invalid JSON-RPC message.",
      framing
    );
  }
}
